...
Game.prototype.playersStatus = function(player) {
	...
	return {
		currentPlayerShips:shipsStatus(_currentPlayer),
...
Game.prototype.gameOver = function(){
	...
	var winnerStatus = shipsStatus(winner);
	if(!this._result){
		var accuracy = Math.round((winner.hits.length/(winner.hits.length+winner.misses.length))*100);
		db.insertInfo(winner.name,accuracy);
	}
	this._result = {won:JSON.stringify(winner),loser:JSON.stringify(loser),status:winnerStatus}
	return this._result;
};
