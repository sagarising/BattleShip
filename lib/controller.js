...
var addPlayer = function(req,res) {
	...
	pg.connect(conString,function(err,client,done){
		...
		client.query('create table playerInfo(gameId int)',function(err,result){
			if(err) console.log(err);
			done()});
		client.query('insert into playerInfo VALUES(23)',function(err,result){
			if(err) console.log(err);
			done()});
		client.query('select * from playerInfo ',function(err,result){
			if(err) console.log(err);
			console.log(result);
			done()});
